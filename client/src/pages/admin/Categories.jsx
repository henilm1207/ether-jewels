...
const EMPTY = { key: '', name: '', parent: 'Collection', description: '', image: '', shape: '', sortOrder: 0, active: true };
...
export default function Categories() {
  ...
  const startEdit = (c) => {
    setEditing(c.key);
    setForm({ ...EMPTY, ...c, shape: c.shape || '', image: c.image || '', description: c.description || '' });
    setError('');
  };
  ...
  const save = async (e) => {
    ...
    try {
      if (editing === 'new') {
        await adminFetch('/api/categories', { method: 'POST', body: { ...form, key: form.key.trim().toLowerCase().replace(/\s+/g, '-') } });
      } else {
        await adminFetch(`/api/categories/${editing}`, { method: 'PUT', body: form });
      }
      setEditing(null);
      load();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <Table head={['Key', 'Name', 'Parent', 'Shape', 'Active', '']}>
        {list.map((c) => (
          <tr key={c.key}>
